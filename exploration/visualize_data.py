...
def heatmap_withpatient(vectors, labels, figname, label_name, scores=None):
    df = pandas.DataFrame({
        'X': vectors[:, 0],
        'Y': vectors[:, 1],
        'label': labels
    ...
    # df.to_csv("heatmap_coordinates_" + figname + ".csv", index=False)

    fig, ax = plt.subplots()
    hmap, x, y = create_heatmap(df)

    hmap_df = pandas.DataFrame(hmap)
    hmap_df.to_csv("coordinates_" + figname + ".csv")
...
def main():
    ...
    # for icu in [1]:
        ...
        heatmap(last_x, y, icu_names[icu] + " ICU - Raw Death Heatmap", data.targets[0])
