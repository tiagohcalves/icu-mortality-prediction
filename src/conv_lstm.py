...
def main():
    # models = ["m3brl2", "m3r_freezeall", "m2", "m6", "m3r_nonfreeze", "m4"]
    models = ["mswitch"]
    for model_id in models:
        ...
        for data_id in ['60']:
            ...
            for n_patients in [1500]:
                ...
                with open("logs/log-" + model_id + "-" + data_id + "-" + str(n_patients) + ".log", "w") as output:
                    ...
                    for shuffle in [False]:
                        ...
                        for use_target in [True]:
                            fine_tunning(model_id, data_id, output, n_patients, shuffle, use_target)
...
def fine_tunning(model_id, data_id, output, n_patients, shuffle, use_target):
            ...
            for fold in range(k_fold):
                ...
                if use_target:
                    score, auc_score = models.evaluate_model(model_name,
                                                             x_test[icu_ids_test == held_out_icu], y_test[icu_ids_test == held_out_icu])
                else:
                    score, auc_score = models.evaluate_model(model_name, x_test, y_test)
                ...
                checkpoint, early_stopping, model = models.create_model(model_name, model.layers)
                ...
                if shuffle:
                    for l in range(len(layers)):
                        if l not in freezable:
                            models.shuffle_weights(model.layers[l])
                ...
                output.write("FT-" + str(held_out_icu) + "-" + str(shuffle) + "-" + str(use_target) + "," +
                             str(fold) + "," +
...
def load_data(data_id,  icus, n_patients):
    ...
    for icu in np.unique(icu_ids):
        ...
        for i, (train_idx, test_idx) in enumerate(skf.split(x_icu, y_icu)):
            x_icu_train, x_icu_val, y_icu_train, y_icu_val = train_test_split(x_icu[train_idx], y_icu[train_idx],
